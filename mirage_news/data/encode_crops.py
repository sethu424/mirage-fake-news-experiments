...
def save_crops(image, boxes, phrases, image_name, split, image_type, output_dir):
    ...
    Args:
        ...
        boxes (Tensor): Bounding boxes for objects (N, 4).
        phrases (list of str): Object class names for each box.
        image_name (str): Name of the source image.
        split (str): Dataset split (e.g., 'train', 'validation').
        image_type (str): 'real' or 'fake'.
        output_dir (str): Root directory to save crops.
    """
...
def process_batch(images, model, device):
    ...
    Args:
        ...
        model: Vision model for encoding.
        device (str): Device to run the model on.

    Returns:
        Tensor: Encoded features for the batch.
    """
...
def process_dataset_or_directory(custom=False, read_dirs=None, output_dir="data/crops", encodings_dir="encodings/crops", batch_size=128):
    """
    Main function to process either a custom directory or the Hugging Face dataset for object detection, cropping, and encoding.

    Args:
        custom (bool): If True, process local directories; otherwise, process Hugging Face dataset.
        read_dirs (list of str): List of directories to read images from (if custom is True).
        output_dir (str): Directory to save cropped images.
        encodings_dir (str): Directory to save encodings.
        batch_size (int): Batch size for encoding crops.
    """
    device = "cuda"
