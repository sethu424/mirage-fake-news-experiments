...
def main(mode, model_class, checkpoint=None):
    """
    Evaluate a trained model (image, text, or multimodal) on one or more test datasets and save the results.

    Args:
        mode (str): Model mode ('image', 'text', or 'multimodal').
        model_class (str): Model class name (e.g., 'linear', 'cbm-encoder', 'mirage').
        checkpoint (str, optional): Path to a specific model checkpoint file. If None, uses the path from config.
    """
    if mode == "multimodal":
        # Load configuration
