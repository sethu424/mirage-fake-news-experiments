...
def create_balanced_sampler(dataset):
    """
    Create a WeightedRandomSampler to balance classes in the dataset.

    Args:
        dataset: Dataset object with a 'labels' attribute (tensor of class labels).

    Returns:
        WeightedRandomSampler or None: Sampler for balanced class sampling, or None if class counts are invalid.
    """
...
def main(mode, model_class):
    """
    Train a model (image, text, or multimodal) using the specified configuration and save the best checkpoint.

    Args:
        mode (str): Model mode ('image', 'text', or 'multimodal').
        model_class (str): Model class name (e.g., 'linear', 'cbm-encoder', 'mirage').
    """
